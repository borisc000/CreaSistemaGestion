...
describe("admin user normalization", () => {
  ...
  it("filters by query, role and tenant", () => {
    const users = [
      normalizeAdminUser(
        buildUserRecord({
          ...
          customClaims: { role: "tenant_admin", tenantId: "tenant-a" }
        })
      ),
      normalizeAdminUser(
        buildUserRecord({
    ...
    const filtered = filterAdminUsers(users, { q: "admin", role: "tenant_admin", tenantId: "tenant-a" });
